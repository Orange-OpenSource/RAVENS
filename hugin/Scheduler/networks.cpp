...
DetailedBlock NetworkNode::compileLayout(bool wantWriteLayout) const
{
	...
	{
		if(wantWriteLayout && isFinal && netToken.destinationBlockID == block)
			continue;
		...
		for (auto token : netToken.sourceToken)
		{
			...
			if(token.length > 0)
				assert(output.fitSegmentInUntagged({token, true}));
		}
	}
