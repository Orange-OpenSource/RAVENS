...
bool forthPassTestWithCompetitiveRead()
{
	...
	const vector<Command> expected = {{REBASE, 0x0, 0x3},
			...
			{CHAINED_COPY, 0x0, 0x0, 0x400},
			{CHAINED_COPY, 0x0, 0x800, 0x400},
			{COPY, 0x0, 0xc00, 0x400, CACHE_BUF, 0x400},
			{COPY, 0x0, 0x400, 0x400, CACHE_BUF, 0xc00},
			{FLUSH_AND_PARTIAL_COMMIT, 0x0, BLOCK_SIZE},
			{RELEASE_BLOCK},
			...
			{COPY, 0x2000, 0x800, 0x400, CACHE_BUF},
			...
			//Exchange [0] and [3]
			{LOAD_AND_FLUSH, 0x0},
			{COPY, CACHE_BUF, 0x400, 0x400, 0x0},
			{CHAINED_COPY, 0x2000, 0x400, 0x400},
			{CHAINED_COPY, CACHE_BUF, 0x0, 0x400},
			{USE_BLOCK, 0x3000},
			{CHAINED_COPY, 0x3000, 0x800, 0x400},
			{COPY, 0x3000, 0x400, 0x400, CACHE_BUF},
			...
			{COPY, 0x1000, 0x800, 0x400, CACHE_BUF},
...
bool forthPassTestWithHarderCompetitiveRead()
{
	...
	const vector<Command> expected = {{REBASE, 0x0, 0x3},
			...
			//Write [0]
			{COPY, 0x0, 0x0, 0x800, CACHE_BUF, 0x800},
			{ERASE, 0x0},
